...
def test_task_status(client, seed_project):
    response = client.get("/projects/1")
    task_1 = "<p>Doing</p>"
    assert task_1 in response.text


def test_task_responsible(client, seed_project):
    response = client.get("/projects/1")
    task_1 = "<p>Responsible 2</p>"
    assert task_1 in response.text


def test_task_description(client, seed_project):
    response = client.get("/projects/1")
    task_1 = "<p>Descrição: Description Task 2</p>"
    assert task_1 in response.text
